...
import apotik.model.KategoriTableModel;

public class KategoriData extends javax.swing.JDialog {

    private Connection connection;
    private KategoriModel kategoriModel;
    private KategoriTableModel kategoriTableModel;
    
    public KategoriData(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        
        connection = DBConnection.newgetInstant();
        kategoriModel = new KategoriModel(connection);
        kategoriTableModel = new KategoriTableModel(new ArrayList<Kategori>());
        tableRender();
    }

    private void tableRender(){
        tableData.setModel(kategoriTableModel);
        tableData.getColumnModel().getColumn(0).setPreferredWidth(40);
        tableData.getColumnModel().getColumn(1).setPreferredWidth(100);
    }
    
    public Kategori getSelectionRow(){
        int index = tableData.getSelectedRow();
        Kategori barang = null;
        if(index >= 0){
            barang = kategoriTableModel.getRows().get(tableData.convertRowIndexToModel(index));
        }
        return barang;
    }
    ...
    private void buttonCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonCariActionPerformed
        kategoriTableModel.setRows(kategoriModel.search(textPencarian.getText()));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(KategoriData.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                KategoriData dialog = new KategoriData(new javax.swing.JFrame(), true);
